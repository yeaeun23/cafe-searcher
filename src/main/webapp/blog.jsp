...
<%
	...
	//XML 데이터를 호출할 URL
	String url = "http://openapi.naver.com/search?key=92598f5d8da0fede511b86aefa037f27&target=blog&display=5&query="+UPSO;
	
	//XML에서 가져올 데이터의 필드명을 배열에 저장
	String[] fieldNames ={"title", "link", "description"};
	
	//아이템 하나에 해당하는 XML 노드를 담을 리스트
	ArrayList<Map> blogList = new ArrayList<Map>();
	...
	try {
		...
		for (int i = 0; i < items.getLength(); i++) 
		{
			...
			for(String name : fieldNames){
				...
				blog.put(name, titleNode.getNodeValue());
			}
			//데이터를 넣은 맵을 리스트에 추가
			blogList.add(blog);
		}
...
<body>
	...
	<div><p name="path">블로그 검색 > <%=UPSO %></p></div>
		...
		<table>
			...
			<tr>
				...
				<td>
					...
					<a href='<%=blog.get("link") %>' target="_blank">
						<img src='./image/link.png' width='20px'>더보기
					</a>
				</td>
			</tr>
		</table>
